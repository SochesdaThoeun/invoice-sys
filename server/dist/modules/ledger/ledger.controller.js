...
class LedgerController {
    constructor() {
         ...
         */
        this.getAllLedgerEntries = async (req, res) => {
            try {
                ...
                const sourceTypeEnum = sourceType && Object.values(LedgerEntry_1.SourceType).includes(sourceType)
                    ? sourceType
                    : undefined;
                const ledgerEntries = await this.ledgerService.getLedgerEntries(sellerId, {
         ...
         */
        this.getLedgerEntryById = async (req, res) => {
            try {
                const sellerId = req.user.id;
                const { id } = req.params;
         ...
         */
        this.createManualEntry = async (req, res) => {
            try {
                const sellerId = req.user.id;
                ...
                const entries = await this.ledgerService.createManualEntries(sellerId, debitEntry.categoryId, creditEntry.categoryId, debitEntry.amount, description);
        ...
        this.getFinancialSummary = async (req, res) => {
            try {
                ...
                const startDateObj = startDate ? new Date(startDate) : undefined;
                const endDateObj = endDate ? new Date(endDate) : undefined;
                const summary = await this.ledgerService.getFinancialSummary(sellerId, startDateObj, endDateObj);
         ...
         */
        this.getProfitAndLoss = async (req, res) => {
            try {
                const sellerId = req.user.id;
                const { startDate, endDate } = req.query;
                // Parse dates if provided
                const startDateObj = startDate ? new Date(startDate) : undefined;
                const endDateObj = endDate ? new Date(endDate) : undefined;
                const profitAndLoss = await this.ledgerService.getProfitAndLoss(sellerId, startDateObj, endDateObj);
         ...
         */
        this.getBalanceSheet = async (req, res) => {
            try {
                const sellerId = req.user.id;
                const { asOfDate } = req.query;
                // Parse date if provided
                const asOfDateObj = asOfDate ? new Date(asOfDate) : new Date();
                const balanceSheet = await this.ledgerService.getBalanceSheet(sellerId, asOfDateObj);
         ...
         */
        this.getIncomeStatement = async (req, res) => {
            try {
                const sellerId = req.user.id;
                ...
                const startDateObj = startDate ? new Date(startDate) : undefined;
                const endDateObj = endDate ? new Date(endDate) : undefined;
                const incomeStatement = await this.ledgerService.getIncomeStatement(sellerId, startDateObj, endDateObj, groupBy);
