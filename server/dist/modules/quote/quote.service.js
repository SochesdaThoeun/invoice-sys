...
class QuoteService {
    constructor() {
        ...
        this.create = async (quoteData, orderCarts, paymentTypeId) => {
            ...
            try {
                ...
                if (savedQuote.status === 'SENT') {
                    if (savedOrder) {
                        await this.ledgerService.createOrderEntries(savedOrder.sellerId, savedOrder.id, Number(totalAmount), `#${savedOrder.id} for quote ${savedQuote.id}`);
                    }
                    else {
                        await this.ledgerService.createQuoteEntries(savedQuote.sellerId, savedQuote.id, Number(savedQuote.totalEstimate), `#${savedQuote.id} for customer ${savedQuote.customerId}`);
                    }
                }
                await queryRunner.commitTransaction();
                return this.findById(savedQuote.id, savedQuote.sellerId);
            }
            catch (error) {
        ...
        this.update = async (id, sellerId, quoteData, orderCarts, paymentTypeId) => {
            // Start a transaction to ensure data consistency
            const queryRunner = data_source_1.AppDataSource.createQueryRunner();
            await queryRunner.connect();
            await queryRunner.startTransaction();
            try {
                ...
                if (isBecomingSent) {
                    if (updatedQuote.orderId) {
                        await this.ledgerService.createOrderEntries(sellerId, updatedQuote.orderId, Number(updatedQuote.totalEstimate), `#${updatedQuote.orderId} for quote ${updatedQuote.id}`);
                    }
                    else {
                        await this.ledgerService.createQuoteEntries(updatedQuote.sellerId, updatedQuote.id, Number(updatedQuote.totalEstimate), `#${updatedQuote.id} for customer ${updatedQuote.customerId}`);
                    }
                }
                await queryRunner.commitTransaction();
                return this.findById(updatedQuote.id, sellerId);
            }
            catch (error) {
                await queryRunner.rollbackTransaction();
                throw error;
